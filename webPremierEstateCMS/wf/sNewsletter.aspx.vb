...
Imports [class].cls_sNewsletter

Partial Class wf_sNewsletter
    ...
    Private Sub bindContent(ByVal domainRef As String, _
        ...
        dt = getNewsletterList(domainRef, keyword, sortBy, sortType)
        ...
        If dt.Rows.Count > 0 Then
            ...
            result.Append("    <td align=""center"">Time</td>")
            result.Append("  </tr>")
            ...
            For i = 0 To dt.Rows.Count - 1
                ...
                If i + 1 >= firstRow Then
                    ...
                    'result.Append("    <td>")
                    'result.Append("      <a title=""Delete"" href=""javascript:doDelete('" + dt.Rows(i).Item("Email").ToString + "');""><img alt=""Delete"" height=""13"" border=""0"" src=""" + rootPath + "support/image/icon_delete.png"" /></a>")
                    'result.Append("    </td>")
                    result.Append("    <td valign=""top""><div id=""divEmail" + i.ToString + """>" + dt.Rows(i).Item("Email") + "</div></td>")
                  
                    result.Append("    <td valign=""top""><div id=""divReceivedDate" + i.ToString + """>" + Format(dt.Rows(i).Item("InputTime"), "dd MMM yyyy") + "</div></td>")
                    result.Append("  </tr>")
                    ...
                    result.Append(bindMessage(dt.Rows(i).Item("Email").ToString))
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Page.Title = "CMS :: " + Session("domain") + " :: Setting :: Newsletter"
        Dim q_page As String = Request.QueryString("p")

        If Trim(q_page) = "" Then q_page = "1"

        If IsPostBack Then
            Dim _sNewsletter As String = Request.Form("_sNewsletter")
            Dim _sNewsletterDelete As String = Request.Form("_sNewsletterDelete")
            ...
            If Trim(_sNewsletter) = "1" Then
                Response.Redirect(GetEncUrl(_rootPath + "wf/sNewsletter.aspx?p=1&", Hashtable))
            End If

            If Trim(_sNewsletterDelete) <> "" Then
                Dim temp As String = ""

                temp = deleteNewsletter(Session("domainRef").ToString, _sNewsletterDelete)
                If Trim(temp) = "" Then
                    'benar
                    Hashtable("note") = "1 record deleted."
                    Response.Redirect(GetEncUrl(_rootPath + "wf/sNewsletter.aspx?p=" + q_page + "&", Hashtable))
                Else
                    'salah
        ...
        Else
            ...
            If isRefresh = "1" Then
                ...
                urlPaging = GetEncUrl(rootPath + "wf/sNewsletter.aspx?p=@p&", Hashtable)
