...
Imports [class].cls_mWebsiteChecker
Partial Class wf_mWebsiteChecker
    ...
    Private Sub bindContent(ByVal userRef As String, _
        ...
        dt = getTagTypeListWebDev(userRef, keyword, sortBy, sortType)
        ...
        If dt.Rows.Count > 0 Then
            ...
            result.Append("                     <th align=""center"">No</th>")
            result.Append("                     <th align=""center"">Name</th>")
            result.Append("                     <th>E-mail</th>")
            result.Append("                     <th>Phone Number</th>")
            result.Append("                     <th>Company</th>")
            result.Append("                     <th>Website</th>")
            'result.Append("                     <th></th>")
            result.Append("                 </tr>")
            ...
            For i = 0 To dt.Rows.Count - 1
                ...
                If i + 1 >= firstRow Then
                    ...
                    Else
                        result.Append("  <tr id=""tr" + i.ToString + """ class=""cart_item"">")
                    End If
                    result.Append("    <td valign=""top""><div id=""divuserRef" + i.ToString + """>" + dt.Rows(i).Item("userRef").ToString + "</div></td>")
                    result.Append("    <td valign=""top""><div id=""divuserName" + i.ToString + """>" + dt.Rows(i).Item("userName") + "</div></td>")
                    ...
                    result.Append("    <td valign=""top""><div id=""divcompany" + i.ToString + """>" + dt.Rows(i).Item("company") + "</div></td>")
                    result.Append("    <td valign=""top""><div id=""divurlWebsite" + i.ToString + """>" + dt.Rows(i).Item("urlWebsite") + "</div></td>")
                    'result.Append("    <td valign=""top"" align=""center"">")
                    'result.Append("      <a title=""Update"" href=""javascript:showPopup(" + i.ToString + "," + dt.Rows(i).Item("userRef").ToString + ",'" + rootPath + "wf/mWebsiteCheckerPopup.aspx?r=" + dt.Rows(i).Item("userRef").ToString + "');""><img alt=""Update"" height=""13"" border=""0"" src=""" + rootPath + "support/image/icon_update.png"" /></a>&nbsp;")
                    'result.Append("      <a title=""Delete"" href=""javascript:doDelete('" + dt.Rows(i).Item("userRef").ToString + "','" + MyURLEncode(dt.Rows(i).Item("userName")) + "');""><img alt=""Delete"" height=""13"" border=""0"" src=""" + rootPath + "support/image/icon_delete.png"" /></a>")
                    'result.Append("    </td>")
                    result.Append("  </tr>")
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsPostBack Then
            Dim _mWebsiteChecker As String = Request.Form("_mWebsiteChecker")
            Dim _mWebsiteCheckerDelete As String = Request.Form("_mWebsiteCheckerDelete")
            ...
            If Trim(_mWebsiteChecker) = "1" Then
                Response.Redirect(GetEncUrl(_rootPath + "wf/mWebsiteChecker.aspx?p=1&", Hashtable))
            End If

            If Trim(_mWebsiteCheckerDelete) <> "" Then
                Dim temp As String = ""

                temp = deleteTagType(Session("domainRef").ToString, _mWebsiteCheckerDelete)
                If Trim(temp) = "" Then
                    'benar
                    Hashtable("note") = "1 record deleted."
                    Response.Redirect(GetEncUrl(_rootPath + "wf/mWebsiteChecker.aspx?p=" + q_page + "&", Hashtable))
                Else
                    'salah
        ...
        Else
            ...
            If isRefresh = "1" Then
                ...
                urlPaging = GetEncUrl(rootPath + "wf/mWebsiteChecker.aspx?p=@p&", Hashtable)
