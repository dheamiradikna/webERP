...
Partial Class wf_contentTagInput
        ...
        Else
            ...
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    ...
                    result.Append("  <img src=""" + rootPath + "wf/support/displayImage.aspx?m=content&tr=" + tagRef + "&d=" + domainRef + "&ir=" + dt.Rows(i).Item("imgRef").ToString + "&w=0"" style=""width:100%"" />")
                    result.Append("</div>")
    ...
    Private Function bindPicture(ByVal tagRef As String, ByVal domainRef As String, ByVal contentRef As String) As String
        ...
        Else
            ...
            End If
            result.Append("<div class=""fNotif mt5 mb5""><a href=""" + rootPath + "wf/support/uploadForm.aspx?src=contentTagPopup.aspx&m=contentPic&tr=" + tagRef + "&t=m&ir=0&r=" + contentRef + """>Click here</a> to upload.</div>")
            If tagRef = _contentTagRefBackgroundSlider Then
                result.Append("<div style=""font-size: 10px;font-weight: bold"">*Max size 500KB</div>")
            Else
                result.Append("<div style=""font-size: 10px;font-weight: bold"">*Max size 250KB</div>")
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsPostBack Then
            ...
            'section file SIUP
            _fileImgName = Request.Form("_fileImgName")
            _fileImgByte = Request.Form("_fileImgByte")
            ImgBytes = Convert.FromBase64String(_fileImgByte)
            Dim ImgLength As Integer = Encoding.UTF8.GetString(ImgBytes).Length
            'end section file SIUP
            ...
            If Trim(_save) = "1" Or Trim(_saveClose) = "1" Then
                ...
                txtContentDate = Request.Form("txtContentDate")
                ...
                ImgFileRef = Request.Form("_ImgFileRef")
                ...
                If IsNumeric(temp) Then
                    ...
                    If Trim(_save) = "1" Then
                        ...
                        Response.Redirect(GetEncUrl(_rootPath + "wf/contentTag.aspx?tr=" + q_tagRef + "&r=" + temp + "&", Hashtable))
        ...
        Else
            ...
            If Trim(q_ref) <> "" Then
                ...
                'dari sql
                If dt.Rows.Count > 0 Then
                    Dim dtImgFile As New DataTable
                    ...
                    If Not IsDBNull(dt.Rows(0).Item("contentDate")) Then
                        txtContentDate = Format(dt.Rows(0).Item("contentDate"), "dd/MM/yyyy").ToString
                    ...
                    dtImgFile = getImage(Session("domainRef").ToString, imgRef)
                    If dtImgFile.Rows.Count > 0 Then
                        Dim ImgFileExt As String = ""
                        If Not IsDBNull(dtImgFile.Rows(0).Item("fileExt")) Then
                            ImgFileExt = dtImgFile.Rows(0).Item("fileExt")
                        End If

                        If Not IsDBNull(dtImgFile.Rows(0).Item("imgFile")) Then
                            ImgBytes = dtImgFile.Rows(0).Item("imgFile")
                            _fileImgByte = Convert.ToBase64String(ImgBytes)
                            _fileImgName = dtImgFile.Rows(0).Item("ImgFileName") + "." + LCase(ImgFileExt)
                            _fileImgUrl = SaveFileToFolder(domainRef, imgRef, dtImgFile.Rows(0).Item("ImgFileName"), ImgFileExt, ImgBytes, "IMAGE")
                        End If
                    End If
                    ...
                    ltrBtn.Text = "  <a href=""javascript:doDelete('" + MyURLEncode(dt.Rows(0).Item("title")) + "');"" class=""btn btn-md btn-dark""><span>Delete</span></a> " + _
                                  "  <a href=""" + GetEncUrl(_rootPath + "wf/contentTagInput.aspx?tr=" + q_tagRef + "&", Hashtable) + """ class=""btn btn-md btn-dark""><span>Insert New</span></a> "
            ...
            If Trim(q_tagRef) <> "" Then
                ...
                If Trim(q_ref) = "" Then
                    ...
                    Else
                        ...
                        If Trim(txtContentDate) = "" Then txtContentDate = (DateTime.Now.ToString("dd/MM/yyyy"))
            ...
            Dim Result As New StringBuilder
            Result.Append("<a href ="" " + _rootPath + "wf/contentTag.aspx?tr=" + tagRef + """ class=""btn btn-md btn-dark""><span>Close</span></a>")

            ltrClose.Text = Result.ToString
