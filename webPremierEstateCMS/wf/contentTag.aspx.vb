...
Partial Class wf_contentTag
   ...
   Private Function bindDay(ByVal id As String, ByVal value As String, ByVal notifID As String) As String
    ...
    Private Sub bindContent(ByVal domainRef As String, ByVal contentType As String, _
                            ByVal imgSetting As String, ByVal tagRef As String, _
        ...
        dt = getContentList(domainRef, contentType, imgSetting, tagRef, contentDateFr, _
        ...
        If dt.Rows.Count > 0 Then
            ...
            For i = 0 To dt.Rows.Count - 1
                ...
                If i + 1 >= firstRow Then
                    ...
                    result.Append("      <a title=""Update"" href=""javascript:debugger;showPopup(" + i.ToString + "," + dt.Rows(i).Item("contentRef").ToString + ",'" + rootPath + "wf/contentTagPopup.aspx?tr=" + tagRef + "&r=" + dt.Rows(i).Item("contentRef").ToString + "');""><img alt=""Update"" height=""13"" border=""0"" src=""" + rootPath + "support/image/icon_update.png"" /></a>&nbsp;")
                    'result.Append("      <a title=""Update"" href=""" + rootPath + "wf/contentTagInput.aspx?tr=" + tagRef + "&r=" + dt.Rows(i).Item("contentRef").ToString + """><img alt=""Update"" height=""13"" border=""0"" src=""" + rootPath + "support/image/icon_update.png"" /></a>&nbsp;")
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        divTitle.InnerHtml = getTagTypeNameByTagRef(Session("domainRef").ToString, q_tagRef) + " :: " + getTagParentNameRekursif(Session("domainRef").ToString, q_tagRef)
        Page.Title = "CMS :: " + Session("domain") + " :: " + divTitle.InnerHtml
        Dim q_page As String = Request.QueryString("p")

        If Trim(q_page) = "" Then q_page = "1"

        If IsPostBack Then
            Dim _contentTag As String = Request.Form("_contentTag")
            Dim _contentTagDelete As String = Request.Form("_contentTagDelete")
            ...
            If Trim(_contentTag) = "1" Then
                Response.Redirect(GetEncUrl(_rootPath + "wf/contentTag.aspx?tr=" + q_tagRef + "&p=1&", Hashtable))
            End If

            If Trim(_contentTagDelete) <> "" Then
                Dim temp As String = ""

                temp = deleteContent(Session("domainRef").ToString, _contentTagDelete)
                If Trim(temp) = "" Then
                    'benar
                    Hashtable("note") = "1 record deleted."
                    Response.Redirect(GetEncUrl(_rootPath + "wf/contentTag.aspx?tr=" + q_tagRef + "&p=" + q_page + "&", Hashtable))
                Else
                    'salah
        ...
        Else
            ...
            Dim resultPlus As New StringBuilder
            'If tagRef <> _contentContactUs And tagRef <> _contentOurProfile Then
                'resultPlus.Append("<div class=""left mb5 ml10"" id=""plus"">")
                'resultPlus.Append("<a href=""" + _rootPath + "wf/contentTagInput.aspx?tr=" + tagRef + """><img border=""0"" alt=""input"" src=""" + _rootPath + "support/image/icon_add.png"" height=""20"" /></a>")
            resultPlus.Append("<a href=""javascript:showPopup(-1,-1,'" + _rootPath + "wf/contentTagPopup.aspx?tr=" + tagRef + "');""><img border=""0"" alt=""input"" src=""" + _rootPath + "support/image/icon_add.png"" height=""20"" /></a>")

            ltrPlus.Text = resultPlus.ToString
            'Else
            '    ltrPlus.Text = resultPlus.ToString
            'End If
            If Trim(q_x) <> "" Then
            ...
            If isRefresh = "1" Then
                ...
                bindContent(Session("domainRef").ToString, selContentType, selImageSetting, q_tagRef, contentDateFrParam, contentDateToParam, isApprovedParam, isPublishParam, isExpiredParam, q_page, txtKeyword, txtMetaDescription, sortByParam, sortTypeParam, numOfData)
                ...
                urlPaging = GetEncUrl(rootPath + "wf/contentTag.aspx?tr=" + q_tagRef + "&p=@p&", Hashtable)
